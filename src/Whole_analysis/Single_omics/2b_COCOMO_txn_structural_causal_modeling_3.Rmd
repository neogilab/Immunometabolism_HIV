...
## load data
```{r}
originalData <- read.csv("/home/flomik/Desktop/Code-PHD/COCOMO_txn/processing/table_condition_biormarkers.csv")
originalData$X <- NULL
...
output_folder = "/home/flomik/Desktop/Code-PHD/COCOMO_txn/results/networks/BNN"
outname = 'transcriptomics_biomarkers'
```
...
write.csv(rank_genes, "processing/bblearn_find_driver_genes.csv")
```
...
```{r}
write.csv(perm10kDAG$arcs, "processing/table_edges_BNN.csv", quote = FALSE)
#write.csv(perm10kDAG$nodes, "processing/table_nodes_BNN.csv")
```

```{r}
write.dot(perm10kDAG, file = "processing/HIV_BNN_transcriptomics_cytoscape_input.dot")
```

```{r}
cat(readLines("processing/HIV_BNN_transcriptomics_cytoscape_input.dot"), sep = "\n")
```
