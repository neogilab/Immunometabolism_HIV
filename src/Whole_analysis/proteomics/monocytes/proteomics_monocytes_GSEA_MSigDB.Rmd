...
```{r}
name_ana <- "GSEA_proteomics_monocytes_hallmark"
```
...
```{r}
#table_global_2 <- table_global_2[table_global_2$comp %in% c("proteomics_monocytes_CB", "proteomics_norm_monocytes_CB"),]
```
...
```{r}
a <- ggplot(table_model_4, aes(y=Stat,  x= pvalue, size = Nb, color = sign, label = Name))+geom_point()+theme_light()+ labs(title="", x="", y="", fill="")+ theme(
        panel.background = element_blank(),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank())+ scale_color_manual(values=colors)+ylab("Piano Statistics")+xlab("P.value")+ geom_text_repel(box.padding = 0.5, size = 4)+facet_grid( ~ comp)
a

ggsave(paste0("results/figures/", name_ana, "_bubble_plot.pdf"), height = 8, width = 15)
```
