...
for (i in 1:length(comp)) {
  ...
  write.csv(top_table_1[, c(6, 2, 3)], paste0("processing/GSEA/proteomics_norm_spermin_frac_", comp[i],".csv"), quote = FALSE, row.names = F)
}
...
path_table <- paste0("results/LIMMA/LIMMA_spermin_norm_proteomics_frac.csv")
...
path_results <- paste0("results/LIMMA/LIMMA_spermin_norm_proteomics_frac_filt.csv")

write.table(DF_top_table, file = path_results)

path_results <- paste0("results/LIMMA/LIMMA_spermin_norm_proteomics_frac_filt.xlsx")
...
top_table_1$sign <- ifelse(top_table_1$adj.P.Val < 0.1, "yes", "no")
table(top_table_1$sign)
...
## add proteins monocytes
```{r}
d <- read.delim("/home/flomik/Desktop/Code-PHD/COCOMO_txn/results/LIMMA/LIMMA_monocytes_norm_proteomics_frac_filt.csv", sep = " ")
d <- d$Gene_name
```
...
venn.diagram(
  x = list(a, b, d),
  category.names = c("Spermidine vs Ctl" , "Spermine vs Ctl" , "At-risk vs HC-like"),
  filename = 'results/figures/venn_spermidine_spermine_proteomics_V2.png',
  output=TRUE,
  fill = myCol,
...
venn.diagram(
  x = list(a, b, c, d),
  category.names = c("Spermidine vs Ctl" , "Spermine vs Ctl" , "Spermine + Spermidine vs Ctrl", "At-risk vs HC-like"),
  filename = 'results/figures/venn_spermidine_spermine_proteomics.png',
  output=TRUE,
  fill = myCol,
  cat.default.pos = "text",
  imagetype="png"
...
list_genes <- a[a %in% d]
length(list_genes)
write.csv(list_genes, "results/EnrichR_input_overlap_spermidine_monocytes.txt", row.names = FALSE, quote = FALSE)
```
...
```
```{r}
gsea <- read.delim("/home/flomik/Desktop/Code-PHD/COCOMO_txn/results/GSEA/KEGG_overlap_monocytes_spermidine.txt")
gsea <- gsea[gsea$Adjusted.P.value < 0.05,]
gsea$Term <- tolower(gsea$Term)
```

```{r}
gset=loadGSC("/home/flomik/Desktop/Code-PHD/COCOMO_txn/data/GSEA/c2.cp.kegg.v7.5.1.symbols_1_3_5.gmt")
x <- gsub("KEGG\\_", "", names(gset$gsc))
x <- gsub("\\_", " ", x)
x <- tolower(x)
```
...
```{r}
gsea$sub_1 <- ifelse(gsea$Term %in% x, "yes", "no")
gsea$sub_2 <- ifelse(gsea$Term %in% gset2$V1, "yes", "no")
```

```{r}
gsea <- gsea[gsea$sub_2 == "yes",]
```
...
```{r}
gsea$Nb <- gsub("\\/.*", "",gsea$Overlap)
gsea$Nb <- as.numeric(gsea$Nb)
gsea <- gsea[gsea$Nb > 20,]
```
...
```{r}
gsea <- gsea[order(gsea$Adjusted.P.value),]
gsea <- gsea[c(1:30),]
```
...
```{r}
gsea <- gsea[order(gsea$Adjusted.P.value, decreasing = TRUE),]
gsea <- gsea[!gsea$Term %in% c("cardiac muscle contraction", "vascular smooth muscle contraction", "axon guidance", "long-term depression", "oxytocin signaling pathway"),]
gsea$Term <- factor(gsea$Term, levels = gsea$Term)
```

```{r}
a <- ggplot(gsea, aes(y=Term,  x= 1, size = Nb, fill = as.numeric(Nb)))+geom_point(colour="black", pch=21)+theme_light()+ labs(title="", x="", y="", fill="")+ theme(
        panel.background = element_blank(),
        panel.grid.major = element_blank(), 
        axis.text.x=element_blank(),
        panel.grid.minor = element_blank())+ scale_color_manual(values=colors)+ylab("KEGG term")+xlab("")+ scale_fill_viridis_b()
a

ggsave(paste0("results/figures/overlap_monocytes_spermidine_bubble_plot.pdf"), height = 6, width = 5)
```
...
print(vol)
path_fig <- paste0("results/figures/volcano_plot_norm_CSpermidine-CControl_frac_monocytes.pdf")
...
print(vol)
path_fig <- paste0("results/figures/volcano_plot_norm_CSpermine-CControl_frac_monocytes.pdf")
ggsave(path_fig, width = 10, height = 6)
```
## volcano plot
```{r}
...
print(vol)
path_fig <- paste0("results/figures/volcano_plot_norm_CBoth-CControl_frac_monocytes.pdf")
ggsave(path_fig, width = 10, height = 6)
```
...
```{r}
rownames(pca_data) <- pca_data$samples
...
data$LFC_Both_Ctrl[data$FDR_Both_Ctrl > 0.05] <- NA
data$LFC_Spermine_Ctrl[data$FDR_Spermine_Ctrl > 0.05] <- NA
data$LFC_Spermidine_Ctrl[data$FDR_Spermidine_Ctrl > 0.05] <- NA
```
...
condition <- factor(condition, levels = c("spermine", "spermidine", "both"))
```
...
```{r}
pdf("results/figures/Heatmap_proteomics_spermine_spermidine_markers_macrophages.pdf", width = 7, height = 9)
ht = Heatmap(data[,-c(1:25)], 
             ...
             right_annotation = rowAnnotation(LogFC_both = data$LFC_Both_Ctrl,
